...
class PersonalInitiative {
     ...
     * @param iniString
     */
    constructor(playerName, iniString) {
...
class InitiativeListDTO {
     ...
     * @param {Number} calculatedInitiative
     */
    constructor(name, initiative, calculatedInitiative = 0) {
        this.name = name;
...
class SheetSectionDiceSets extends AbstractSheetHelper {
     ...
     * @param {String} dmg
     * @param {Number} bullets
     * @param {Boolean} isSentFromGm
     */
    addDiceSet(name = '', threshold = 0, amount = 0, dmg = '-', bullets = 1, isSentFromGm = false) {
        debug.log("SheetSectionDiceSets.addDiceSet");

        if (isSentFromGm) {
            // Remove all elements with the class "gm-dice-set"
            const gmDiceSets = document.querySelectorAll('.gm-dice-set');
            gmDiceSets.forEach(element => element.remove());
        }

        let diceSetIndex = this.determineNextIndex(this.FIELDNAME_INDEX);
        let gmClass = isSentFromGm ? ' gm-dice-set' : ''; // Add a class if sent from GM
        let iconClass = isSentFromGm ? 'ts-icon-character-gamemaster' : 'ts-icon-d6';

        let newDiceSetHtmlString = ' \
    <div id="dice-sets-cell-icon-dice[' + diceSetIndex + ']" class="grid-item' + gmClass + '" style="grid-column-end: span 1;"> \
        <i class="icon-dice ' + iconClass + ' ts-icon-small" onclick="diceService.rollDices(' + diceSetIndex + ');"></i> \
    </div> \
    <div id="dice-sets-cell-name[' + diceSetIndex + ']" class="grid-item-data align-left' + gmClass + '" style="grid-column-end: span 1;"> \
        <input name="dice-sets-id[]" type="hidden" value="' + diceSetIndex + '"> \
        <input name="dice-sets-name[' + diceSetIndex + ']" class="field-data" type="text" value="' + name + '" onchange="diceService.persistDiceSets();"> \
    </div> \
    <div id="dice-sets-cell-threshold[' + diceSetIndex + ']" class="grid-item-data' + gmClass + '" style="grid-column-end: span 1;"> \
        <input name="dice-sets-threshold[' + diceSetIndex + ']" class="field-data-short" type="number" value="' + threshold + '" onchange="DiceService.preventNegativeValue(this); diceService.persistDiceSets();"> \
    </div> \
    <div id="dice-sets-cell-amount[' + diceSetIndex + ']" class="grid-item-data' + gmClass + '" style="grid-column-end: span 1;"> \
        <input name="dice-sets-amount[' + diceSetIndex + ']" class="field-data-short" type="number" value="' + amount + '" onchange="DiceService.preventNegativeValue(this); diceService.persistDiceSets();"> \
    </div> \
    <div id="dice-sets-cell-dmg[' + diceSetIndex + ']" class="grid-item-data' + gmClass + '" style="grid-column-end: span 1;"> \
    ...
    <div id="dice-sets-cell-bullets[' + diceSetIndex + ']" class="grid-item-data' + gmClass + '" style="grid-column-end: span 1;"> \
        <input name="dice-sets-bullets[' + diceSetIndex + ']" class="field-data-short" type="number" value="' + bullets + '" onchange="DiceService.preventNegativeValue(this); diceService.persistDiceSets();"> \
    </div> \
    <div id="dice-sets-cell-icon-remove[' + diceSetIndex + ']" class="grid-item' + gmClass + '" style="grid-column-end: span 1;"> \
     ...
     * @param isSentFromGm
     */
    addDefenceDiceSet(name = '', threshold = 0, amount = 0, dmg = '-', isSentFromGm = false) {
        debug.log("SheetSectionDiceSets.addDefenceDiceSet");

        // Remove all elements with class "gm-dice-set" if isSentFromGm is true
        if (isSentFromGm) {
            document.querySelectorAll('.gm-dice-set').forEach(element => element.remove());
        }

        let diceSetIndex = this.determineNextIndex(this.DEFENCE_FIELDNAME_INDEX);
        let gmClass = isSentFromGm ? ' gm-dice-set' : ''; // Add a class if sent from GM

        let newDiceSetHtmlString = ' \
            <div id="defence-dice-sets-cell-icon-dice[' + diceSetIndex + ']" class="grid-item' + gmClass + '" style="grid-column-end: span 1;"> \
                <i class="' + (isSentFromGm ? 'ts-icon-character-gamemaster' : 'icon-dice ts-icon-d6 ts-icon-small') + '" onclick="diceService.rollDefenceDices(' + diceSetIndex + ');"></i> \
            </div> \
            <div id="defence-dice-sets-cell-name[' + diceSetIndex + ']" class="grid-item-data align-left' + gmClass + '" style="grid-column-end: span 1;"> \
                <input name="defence-dice-sets-id[]" type="hidden" value="' + diceSetIndex + '"> \
                <input name="defence-dice-sets-name[' + diceSetIndex + ']" class="field-data" type="text" value="' + name + '" onchange="diceService.persistDiceSets();"> \
            </div> \
            <div id="defence-dice-sets-cell-threshold[' + diceSetIndex + ']" class="grid-item-data' + gmClass + '" style="grid-column-end: span 1;"> \
                <input name="defence-dice-sets-threshold[' + diceSetIndex + ']" class="field-data-short" type="number" value="' + threshold + '" onchange="DiceService.preventNegativeValue(this); diceService.persistDiceSets();"> \
            </div> \
            <div id="defence-dice-sets-cell-amount[' + diceSetIndex + ']" class="grid-item-data' + gmClass + '" style="grid-column-end: span 1;"> \
                <input name="defence-dice-sets-amount[' + diceSetIndex + ']" class="field-data-short" type="number" value="' + amount + '" onchange="DiceService.preventNegativeValue(this); diceService.persistDiceSets();"> \
            </div> \
            <div id="defence-dice-sets-cell-dmg[' + diceSetIndex + ']" class="grid-item-data' + gmClass + '" style="grid-column-end: span 1;"> \
                <input name="defence-dice-sets-dmg[' + diceSetIndex + ']" class="field-data-short" type="text" value="' + dmg + '" onchange="diceService.persistDiceSets();"> \
            </div> \
            <div id="defence-dice-sets-cell-icon-remove[' + diceSetIndex + ']" class="grid-item' + gmClass + '" style="grid-column-end: span 1;"> \
